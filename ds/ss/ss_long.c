 ...
 */
inline void SWAP(long *a, int j, int k)
{
    long temp = a[j];  a[j] = a[k];  a[k] = temp;
}

int default_comparator(long a, long b)
{
...
{
    return long_linearsearch_with(default_comparator,a,k,N);
}

int long_binarysearch(long a[], long k, int N)
{
    return long_binarysearch_with(default_comparator,a,k,N);
}

int long_linearsearchX(long a[], long k, int N)
{
    return long_linearsearchX_with(default_comparator,a,k,N);
}

void long_bubblesort(long a[], int N) { long_bubblesort_with(default_comparator,a,N); }
void long_selectsort(long a[], int N) { long_selectsort_with(default_comparator,a,N); }
void long_insertsort(long a[], int N) { long_insertsort_with(default_comparator,a,N); }
void long_heapsort(  long a[], int N) { long_heapsort_with(  default_comparator,a,N); }
void long_quicksort( long a[], int N) { long_quicksort_with( default_comparator,a,N); }
 ...
 */
int smallest(comparator compare, long a[], int lo, int N)
{
    int s = lo, i;
    for (i=lo+1; i<N; i++)
 ...
 */
void insert(comparator compare, long a[], long x, int p)
{
    int i = 0, j;
 ...
 */
void siftDown(comparator compare, long a[], int k, int N)
{
    int parent=k;
    int child;
    while(1) {
...
void makeHeap(comparator compare, long a[], int N)
{
    int k = (N-1) / 2;
    while (k>=0) {
...
* Quick sort                                                                              *
******************************************************************************************/
void q_sort(comparator compare, long *a, int left, int right, int N)
{
    int i, last, pivot;
    if (left >= right)
        return;
