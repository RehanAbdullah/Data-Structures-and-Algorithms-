// Auto-generated impl. file for a bst of long
...
struct bst_long_implementation
{
    struct node * root;
    int size;
};
...
{
    bst_long * t = (bst_long*)malloc(sizeof(bst_long));
    t->root = NULL;
    t->size = 0;
    return t;
}
...
struct node * ins(bst_long *t, long x, struct node *p)
{
    if (p==NULL) {
        p = (struct node *)malloc(sizeof(struct node));
        if (p==NULL) {
            printf("bst_longinsert (ins): failed to allocate memory\n");
            exit(1);
        }
        p->left = p->right = NULL;
        p->item = x;
...
{
    if (p==NULL)
        printf("error: (bst_long) findmin: empty tree\n");
...
struct node * del(bst_long *t, long x, struct node *p)
{
    struct node * n;
...
int isin(bst_long *t, long x, struct node *p)
{
    if (p==NULL)
        return 0;
...
void preorder(struct node *p, void (* item_print)(long item))
{
    if (p!=NULL) {
        item_print(p->item);
...
void inorder(struct node *p, void (* item_print)(long item))
{
    if (p!=NULL) {
...
void postorder(struct node *p, void (* item_print)(long item))
{
    if (p!=NULL) {
...
void simple_print(struct node *p, int ind, void (* item_print)(long item))
{
    int i;
