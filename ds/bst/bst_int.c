// Auto-generated impl. file for a bst of int
...
bst_int * new_bst_int()
{
    bst_int * t = (bst_int*)malloc(sizeof(bst_int));
    t->root = NULL;
    t->size = 0;
    return t;
}
...
struct node * ins(bst_int *t, int x, struct node *p)
{
    if (p==NULL) {
        p = (struct node *)malloc(sizeof(struct node));
        if (p==NULL) {
            printf("bst_intinsert (ins): failed to allocate memory\n");
            exit(1);
        }
        p->left = p->right = NULL;
        p->item = x;
...
{
    if (p==NULL)
        printf("error: (bst_int) findmin: empty tree\n");
...
struct node * del(bst_int *t, int x, struct node *p)
{
    struct node * n;
...
int isin(bst_int *t, int x, struct node *p)
{
    if (p==NULL)
        return 0;
...
void preorder(struct node *p, void (* item_print)(int item))
{
    if (p!=NULL) {
        item_print(p->item);
...
void inorder(struct node *p, void (* item_print)(int item))
{
    if (p!=NULL) {
...
void postorder(struct node *p, void (* item_print)(int item))
{
    if (p!=NULL) {
...
void simple_print(struct node *p, int ind, void (* item_print)(int item))
{
    int i;
